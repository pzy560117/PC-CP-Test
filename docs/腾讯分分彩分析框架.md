...
```mermaid
flowchart TD
    Raw[lottery_draws] --> Stat[statistics.py\n频率分析/Markov]
    Raw --> Backtest[backtest.py\n均线策略]
    Stat --> Features[lottery_features\nfrequency/randomness/markov]
    Backtest --> Features
    Stat --> Results[analysis_results\nstatistical_analysis]
    Backtest --> Results
    Results --> API[/FastAPI Export API/]
    API --> Frontend[React Analysis & History Pages]
```

- **统计/状态模块**：新增 `statistics.py`，执行数字频率、卡方与 Markov 转移矩阵分析；结果通过 `statistical_analysis` 类型写入 `analysis_results`，并落入 `lottery_features` 便于缓存/导出。
- **策略回测模块**：`backtest.py` 实现和值均线策略，将 PnL、胜率、equity curve 写入 `strategy_backtest` 结果，供前端展示趋势表现。
- **Worker 并发优化**：`jobs_worker.py` 使用线程池批处理，记录 batch 耗时并支持配置最大并行度，保证统计与策略任务在高吞吐场景下稳定。
- **导出与批量查询**：FastAPI 暴露 `/api/v1/analysis/results/export`、`/api/v1/features/batch|export`，支持 JSON/CSV 导出及多期批量返回；DB 层通过 expanding bindparam 与缓存减少重复查询。
- **前端消费**：`analysis.tsx` 绘制 Sparkline、Markov 矩阵与策略摘要，`history.tsx` 支持筛选、勾选导出与特征详情，下游直接复用上述导出接口。
