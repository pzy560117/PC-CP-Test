...
flowchart LR
    ...
    RAW[(lottery_draws)] --> S1
    RAW --> S2
    S1 -->|lottery_features\nanalysis_results| DB[(MySQL)]
    S2 -->|strategy_backtest| DB
    DB --> API[FastAPI 导出 API]
    API --> FE[React 前端\nDashboard/Analysis/History]
```

- **统计模块**：`analysis/src/statistics.py` 聚合最近 N 期数据计算数字频率、卡方与 Markov 转移矩阵，并写入 `lottery_features` 与 `analysis_results(statistical_analysis)`。
- **策略回测模块**：`analysis/src/backtest.py` 实现和值均线策略，产生 `strategy_backtest` 结果与 `strategy_backtest` 特征，支持前端展示信号与收益。
- **并发任务执行**：`analysis/src/jobs_worker.py` 使用 `ThreadPoolExecutor` 批量执行任务，记录 batch 成功/失败、平均耗时及并行度指标。
- **导出/批量查询**：`backend/app/api/analysis.py` 暴露 `/api/v1/analysis/results/export` 与 `/api/v1/features/batch|export`，提供 JSON/CSV 导出与多期特征查询。
- **前端展示**：`frontend/src/pages/analysis.tsx` 展示 Sparkline、Markov 矩阵与策略回测摘要；`frontend/src/pages/history.tsx` 支持期号筛选、勾选导出与特征详情预览。
